...
/** 
 * 自定义的指定Shiro验证用户登录的类 
 * @see 在本例中定义了2个用户:jadyer和xuanyu,jadyer具有admin角色和admin:manage权限,xuanyu不具有任何角色和权限 
...
public class MyRealm extends AuthorizingRealm {  
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals){  
    	log.info("doGetAuthorizationInfo in MyRealm-----------------dyp");
...
/*      List<String> roleList = new ArrayList<String>();  
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {  
    	log.info("doGetAuthenticationInfo in MyRealm-----------------dyp");
        //获取基于用户名和密码的令牌  
        //实际上这个authcToken是从currentUser.login(token)传过来的 ,而在Web程序中是由authc对应的对象调 用的
        ...
        //==========================================================================================
        //此处无需比对,只需把从数据库里查出来的密码写入AuthenticationInfo对象中,比对的逻辑Shiro会做
        //即我们只需返回一个和令牌相关的正确的验证信息  
        ...
        if("jadyer".equals(token.getUsername())){  
            ...
            this.setSession("currentUser", "xuanyu");  
            return authcInfo;  
        }  
